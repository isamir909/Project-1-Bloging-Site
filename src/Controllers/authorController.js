...
const createAuthor = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: " input object can not be empty" });
    ...
    for (let i = 0; i < array1.length; i++) {
      ...
      if (type == "object")
        return res
          .status(400)
          .send({ status: false, msg: "input data can not be null" });
      if (type != "string")
        return res
          .status(400)
          .send({ status: false, msg: "input data must be string" });
      if (array1[i].toLowerCase() == "undefined")
        return res
          .status(400)
          .send({ status: false, msg: "input data can not be undefined" });
    ...
    if (LnameValidate == false || FnameValidate == false)
      return res
        .status(400).send({status: false,msg: "LastName and firstName must be between A-z or a-z ",});
    ...
    if (checkAuthor)
      return res
        .status(400)
        .send({ status: false, msg: "this email is already in use" });
